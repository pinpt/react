...
test('Test full page default closed', () => {
	const component = renderer.create(<PrebuiltRoadmap site={site} roadmap={roadmap} initialOpen={false} />);
	const tree = component.toJSON();
	expect(tree).toMatchSnapshot();
});

test('Test full page default empty', () => {
	const component = renderer.create(<PrebuiltRoadmap site={site} roadmap={empty_roadmap} />);
	const tree = component.toJSON();
	expect(tree).toMatchSnapshot();
});

test('Test custom className', () => {
	const component = renderer.create(<PrebuiltRoadmap className="test-custom" site={site} roadmap={roadmap} />);
	const tree = component.toJSON();
	expect(tree).toMatchSnapshot();
});
